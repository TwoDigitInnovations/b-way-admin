  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full lg:w-auto">
        <Header sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} title="All Dispatchers" />
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[800px]">
                <thead className="bg-[#003C72] text-white">
                  <tr>
                    <th className="w-16 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">No.</th>
                    <th className="w-40 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Name</th>
                    <th className="w-48 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Email</th>
                    <th className="w-32 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Phone</th>
                    <th className="w-32 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Role</th>
                    <th className="w-28 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Status</th>
                    <th className="w-20 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Action</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {dispatchersData.map((dispatcher, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm text-gray-900">{dispatcher.no}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 font-medium">{dispatcher.name}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{dispatcher.email}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{dispatcher.phone}</td>
                      <td className="px-4 py-3 text-sm text-gray-900">{dispatcher.role}</td>
                      <td className="px-4 py-3 text-sm">
                        <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-md ${getStatusStyle(dispatcher.status)}`}>
                          {dispatcher.status}
                        </span>
                      </td>
